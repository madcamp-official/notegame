...
키보드 스킬 `COPY`, `DELETE`, `CONNECT`, `RESTORE`, `UNDO`, `SEARCH`, `SELECT_ALL`은 이 문맥 안에서 사용됩니다. `Ctrl C`는 오브젝트를 복제해 `Ctrl V`로 빈 타일에 배치하고 조사 효과는 없습니다. `Delete`는 선택한 적 하나를 공격하며, `Ctrl F`는 선택한 대상 하나를 조사합니다. `Ctrl Z`는 최근의 소비되지 않은 가역 턴 2개에 보상 연산을 적용하되 현재 턴, version, D20과 이력은 단조 증가 상태로 유지합니다. `Ctrl A`는 반경 4칸의 모든 적을 공격합니다. 위험 지역 이동은 안전 구간 도착과 조우 활성화까지만 처리하고, 이후 실제 행동에서 D20과 캠페인 턴을 소비합니다.
