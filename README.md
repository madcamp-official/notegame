...
기본 모델은 신규 프로젝트에서 호출 가능한 Flash-Lite 설정인 `gemini-3.1-flash-lite`입니다. 최소 thinking level, 제한된 구조화 출력, 최대 1회 repair와 deterministic fallback을 사용합니다. 타이틀의 설정 화면에서 플레이어가 Gemini API 키를 직접 입력할 수 있으며, 입력한 키는 이 기기의 Unity `PlayerPrefs`에 저장되어 요청 전용 헤더로 로컬 서버에 전달됩니다. 입력하지 않으면 서버 환경 변수 `GEMINI_API_KEY`를 사용합니다. 서버는 키를 응답, 로그, DB plan 또는 snapshot에 기록하지 않습니다.
...
Gemini 연동은 게임의 설정 화면에서 키를 입력하거나 로컬 `Server/.env`에 설정할 수
있습니다. `.env`와 DB 비밀번호는 어떤 산출물에도 포함되지 않습니다.
