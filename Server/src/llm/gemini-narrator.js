...
import { fallbackPlayerActionProposal, PLAYER_ACTION_KINDS, validatePlayerActionProposal } from "./player-action.js";
...
export class GeminiNarrator {
  constructor({ apiKey = "", timeoutMs = 15000, fetchImpl = globalThis.fetch, logger = console, modelProfiles = DEFAULT_MODEL_PROFILES, responseTrace = null } = {}) {
    this.apiKey = apiKey;
  ...
  async planPlayerAction(context) {
    if (!this.apiKey || typeof this.fetchImpl !== "function") return fallbackPlayerActionProposal(context);
    ...
    try {
      ...
      if (proposal && PLAYER_ACTION_KINDS.includes(String(proposal.kind || "").toUpperCase())
        ...
        const reason = kind === "COMBINE" && (!Array.isArray(proposal.itemIds) || proposal.itemIds.length < 2)
          ? "조합에 필요한 소지품을 실제 인벤토리에서 모두 찾을 수 없다."
          : kind === "USE_ITEM" && (!Array.isArray(proposal.itemIds) || proposal.itemIds.length < 1)
            ? "사용하려는 물건이 실제 인벤토리에 없다."
            : "현재 장면과 시스템 상태에서는 이 시도를 실행할 수 없다.";
        return {
          ...fallback,
  ...
  async planSkillTarget(context) {
    ...
    if (!this.apiKey || typeof this.fetchImpl !== "function") return createFallbackSkillTarget(context);
  ...
  async resolveArc(context) {
    ...
    if (!this.apiKey || typeof this.fetchImpl !== "function") return { fallbackUsed: true };
  ...
  async generateOnce(context, profile) {
    const prompt = composeNarrationPrompt(context, STORY_SEQUENCE_PROMPT + TURN_SYSTEM_PROMPT + CONFIRMED_EFFECTS_PROMPT);
  ...
  async reviewNarrativeScene(context, output, profile) {
    ...
    const review = await this.requestJson({
      systemText: NARRATIVE_CHOICE_REVIEW_PROMPT,
      userPayload: {
        readOnlyContinuity: {
