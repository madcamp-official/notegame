...
function validateDirectorContext(input) {
  ...
  assert(["COPY", "DELETE", "CONNECT", "RESTORE", "UNDO", "SEARCH", "SELECT_ALL"].includes(input.skillId), 400, "NARRATION_CONTEXT_INVALID", "skillId must be a keyboard skill.");
  ...
  assert(input.activeQuests.length <= 6 && input.visibleEntities.length <= 32 && input.placementSlots.length <= 8 && input.readOnlyPlaces.length <= 24 && input.readOnlySlots.length <= 12 && input.canonicalFacts.length <= 16
    && input.majorChoices.length <= 8 && input.regionOutcomes.length <= 6 && input.abilityUsageHistory.length <= 8 && input.adminAccessHistory.length <= 3 && input.technicalDebtEntries.length <= 8 && input.unresolvedHooks.length <= 8 && input.sceneSequence.length <= 16,
