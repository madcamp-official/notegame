...
export function validateSceneDirectorContext(input) {
  ...
  exactKeys(input, ["schemaVersion", "requestType", "decisionType", "decisionNo", "campaign", "macroPhase", "storyBeat", "area", "playerId", "candidates", "recentSceneTypes", "openLoops", "npcRelationships", "fixedCanon", "maxSelectedActions"], "SCENE_CONTEXT_INVALID", 400);
  ...
  for (const candidate of input.candidates) {
    ...
    exactKeys(candidate, ["candidateId", "type", "actorId", "targetId", "priority", "cost", "reason", "actionStyle", "slotId", "assetId", "spawnKind", "displayName", "traitIds", "rewardId", "questId", "pendingId", "delta"], "SCENE_CONTEXT_INVALID", 400);
...
export function validateScenePlan(input, contextInput) {
  ...
  const allowedSpeakers = new Set(context.candidates.flatMap((candidate) => [candidate.actorId, candidate.targetId]).filter(Boolean));
...
export const SCENE_PLAN_RESPONSE_JSON_SCHEMA = Object.freeze({
  type: "object",
  additionalProperties: false,
  required: ["sceneGoal", "selectedActionIds", "dialogue"],
  properties: {
    sceneGoal: { type: "string" },
    selectedActionIds: { type: "array", minItems: 1, maxItems: MAX_SELECTED_ACTIONS, items: { type: "string" } },
